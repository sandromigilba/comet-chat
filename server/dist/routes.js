...
router.post('/api/register', async (req, res) => {
    ...
    try {
        ...
        const [existingUsers] = await pool.query('SELECT id FROM users WHERE username = ?', [normalizedUsername]);
...
router.post('/api/login', async (req, res) => {
    ...
    try {
        // Find user
        const [users] = await pool.query('SELECT * FROM users WHERE username = ?', [normalizedUsername]);
        if (users.length === 0) {
...
router.get('/api/users/search', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const [users] = await pool.query('SELECT username FROM users WHERE username LIKE ? AND username != ? LIMIT 15', [searchPattern, excludeUsername || '']);
...
router.get('/api/threads/recent', authenticateToken, async (req, res) => {
...
router.get('/api/messages', authenticateToken, async (req, res) => {
    const usernameA = req.query.a;
    const usernameB = req.query.b;
    ...
    try {
        ...
        const [messages] = await pool.query('SELECT * FROM messages WHERE threadId = ? ORDER BY id ASC', [threadId]);
...
router.post('/api/messages', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const [result] = await pool.query('INSERT INTO messages (threadId, senderUsername, recipientUsername, content, createdAt) VALUES (?, ?, ?, ?, ?)', [threadId, senderUsername, recipientUsername, content, createdAt]);
